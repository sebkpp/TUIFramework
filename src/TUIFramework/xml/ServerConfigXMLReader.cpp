	...
	static Port extractPort(DOMDocument * domDocument, DOMTreeWalker * domTreeWalker) {
		...
		while (node)
		{
			...
			else if (XMLString::compareString(XMLString::transcode(node->getNodeName()), "ParameterGroup") == 0)
			{
				...
				if (XMLString::compareString(XMLString::transcode(nameAttribute->getNodeValue()), "Constraint") == 0) {
					...
					if (XMLString::compareString(XMLString::transcode(node->getNodeName()), "Parameter") == 0) {
						for (int index = 0; index < 2; index++) {
							...
							if (XMLString::compareString(XMLString::transcode(nameAttribute->getNodeValue()), "min") == 0) {
								if (!value.empty())
							...
							else if (XMLString::compareString(XMLString::transcode(nameAttribute->getNodeValue()), "max") == 0) {
								if (!value.empty())
				...
				else if (XMLString::compareString(XMLString::transcode(nameAttribute->getNodeValue()), "Meta") == 0) {
					node = domTreeWalker->nextNode();
