...
CLSID CRemoteHost::GetCLSIDFromRemoteRegistry(const CString & hostName, const CString &progID)
{
   ...
   if (SUCCEEDED(result)){
       result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, keyName, 0, KEY_READ, &keyHandle);
	   if (SUCCEEDED(result)){
		   DWORD entryType;
		   DWORD dwReturn;
		   unsigned bufferSize = 100;
           //result = RegQueryValueEx(keyHandle, NULL, 0, &entryType, (LPBYTE)&classIdString, (LPDWORD)&bufferSize);
		   result = RegQueryValueEx(keyHandle, NULL, 0, 0, (LPBYTE)&classIdString, (LPDWORD)&bufferSize);
           if (FAILED(result)){
			printf("here");
		   }else{
			   USES_CONVERSION;
			   //CString test(classIdString);
			   
			   //std::cout << test.GetLength() << " " << test << std::endl;
